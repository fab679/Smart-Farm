...
import { verifyPickUpStationLogin } from "~/models/pickupstation.server";

export const loader: LoaderFunction = async ({ request }) => {
  const userId = await getPickUpId(request);
  if (userId) return redirect("/pickup");
  return json({});
};
...
export const action: ActionFunction = async ({ request }) => {
  ...
  const user = await verifyPickUpStationLogin(email, password);
...
export const meta: MetaFunction = () => {
  return {
    title: "PickUp Station Login",
  };
};

export default function LoginPage() {
  const [searchParams] = useSearchParams();
  const redirectTo = searchParams.get("redirectTo") || "/pickup";
